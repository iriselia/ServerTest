...
PreparedStatement::PreparedStatement(StatementStorage _storage, uint32 _index, ParamBind* _paramTypeInfo)
...
void PreparedStatement::SetParametersypeInfo(ParamBind* _parameter)
{
	ParametersBuffer = std::unique_ptr<ParamBind>(std::move(_parameter));
}

void PreparedStatement::SetBool(const uint8 index, const bool value)
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_BOOL);
	ParametersBuffer->SetParam<bool>(index, std::move(value));
}

void PreparedStatement::setUInt8(const uint8 index, const uint8 value)
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_UI8);
...
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_UI16);
...
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_UI32);
...
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_DOUBLE);
	ParametersBuffer->SetParam<double>(index, std::move(value));
}

void PreparedStatement::setString(const uint8 index, const char* value)
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_STRING);
...
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_BINARY);
	ParametersBuffer->SetParam<const char*>(index, (const char*)value.data());
}

void PreparedStatement::setNull(const uint8 index)
{
	ASSERT(ParametersBuffer->GetParamType(index) == QueryParamCPPType::TYPE_NULL);
